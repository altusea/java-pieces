...
class LazyValueImpl<T> implements LazyValue<T> {
    ...
    /**
     * VarHandle's for atomic access to {@code theLock} and {@code loaded} instance variables.
     */
    ...
    /**
     * Semaphore to prevent concurrent update of internal state. Updated only via {@code THE_LOCK}.
     */
    private volatile Semaphore theLock;

    /**
     * Boolean indicating value if readily available without calling a supplier.
     */
